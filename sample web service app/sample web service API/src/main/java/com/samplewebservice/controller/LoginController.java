...
public class LoginController {
	...
	public ResponseEntity<Map<String, Object>> login(@RequestBody UserLoginJSON userLoginJSON,
		...
		if(userLoginJSON != null){
			 userDTO = loginService.login(userLoginJSON.getEmailId(), userLoginJSON.getPassword());// make call to login service
		}
			if(userDTO != null){
				//if user is valid
				...
				responseObj.put("message", "Login Failed, Please ensure that username and password is correct");
				return new ResponseEntity<Map<String, Object>>(responseObj, HttpStatus.OK);
			}			
	}
